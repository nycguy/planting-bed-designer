...
4. **Review** — a map fitted to all beds, each filled in its color with a bed-number and area label, then every bed's numbers and combined totals. "Beds are finished" is the only way forward.
...
6. **Plants** — the planting design. Pick a category (Trees, Shrubs, Flowers & bulbs), search a list filtered to plants hardy in the property's zone, choose one, and tap the map to place it. Trees and shrubs draw as circles at mature spread; flowers and bulbs as roughly 18-inch irregular drifts. Circles may overlap (a shrub under a tree canopy is normal). Tap a placed plant to move or remove it. The sheet lists what is placed in each bed.
7. **Complete** — the report: address, hardiness zone, map with all outlines and plants, totals, a plant list by bed with quantities and mature sizes, and a section per bed with photo and measurements. Print, Save as PDF, Save design file, Export design data (GeoJSON), edit anything, or start over.
...
- `npm run test:unit`: 40 tests, passing (geometry math, side-length editing semantics, stage gating, status derivation, transfer round-trip, in-app browser detection, App mounts on the Beds stage with no photo inputs).
